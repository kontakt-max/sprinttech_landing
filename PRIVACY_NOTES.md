...
Endpoint `/api/threat-pulse` **nie przetwarza danych użytkownika**. Pobiera wyłącznie publiczne dane CVE/EPSS po stronie serwera. Odpowiedź nie zawiera PII ani raw indicators (IP, domeny).

Scenariusze w hero command center to **symulowany przebieg procesu** SprintTech — nie telemetria klientów.
