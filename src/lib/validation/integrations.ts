...
export const soraVideoRequestSchema = z.object({
  prompt: z.string().min(10).max(1000),
  duration: z.number().min(1).max(60).optional(),
  aspectRatio: z.enum(["16:9", "9:16", "1:1"]).optional(),
  consentModeration: z.literal(true, {
    errorMap: () => ({ message: "Wymagana zgoda na moderację treści" }),
  }),
});

export type SoraVideoRequest = z.infer<typeof soraVideoRequestSchema>;
